这是一份计算机网络笔记，起初用于考试和面试，现在也用于记录一些工作和生活上遇到的网络问题，笔记的内容大部分整理自一些已经遗失的课件以及《TCP/IP 详解：协议》一书。
...
UDP 中的 D 指的是 **datagram**（数据报）

进程到进程的**数据交付和差错检查**是两种最低限度的运输层服务，**UDP所能提供的仅有的两种服务**。

通过使用流量控制，序号，确认和定时器，确保**正确地，按序地**将数据从发送进程交付给接受进程。从而TCP成为了可靠的额数据传输协议。

### 多路复用和多路分解
**运输层的多路复用（transport-layer multiplexing）与多路分解（demultiplexing）**：将主机间交付扩展到进程间交付。

将由网络层提供的主机到主机交付的服务，延伸到为运行在主机上的应用程序提供的进程到进程的交付服务。

在**接收主机**中的运输层实际上并没有直接将数据交付给进程，而是将数据交给了一个中间的**套接字**。任意时刻，接收主机上可能不止有一个套接字，所以每个套接字都有唯一的标识符。标识符的格式取决于它是UDP还是TCP套接字。

将运输层报文段中的数据交付到正确的套接字的工作称为**多路分解（demultiplexing）**（从下至上）

在源主机从不同套接字中收集数据块，并为每个数据块封装上首部信息从而生成报文段，然后将报文段传递到网络层，所有这些工作称为**多路复用（multiplexing）**（从上至下）
...
- 点到点:
    ...
    - 连接状态与端系统有关，不为路由器所知 
- 可靠、有序的字节流
	- 没有 “报文边界”：这意味着 TCP 可能会出现 **粘包问题(黏包问题)**，而 UDP 是有报文边界的，因此不会出现这个问题。
- 流水线:
	- TCP拥塞和流量控制设置滑动窗口协议
- 发送和接收缓冲区
- 全双工数据:
...
### ARP协议
链接互联网的设备一般都有一个网卡，网卡由一个物理地址进行标识（通常是MAC地址），和动态划分的IP地址不同，这个物理地址是唯一且不变的，在链路层需要使用MAC地址进行通讯，这需要一个能通过IP进行MAC地址映射的协议，这个协议就是ARP协议。

ARP协议：地址解析协议，提供了一种在IPv4地址和各种网络技术使用的硬件地址之间的映射（通常是IP地址 -> MAC地址）。ARP仅用于IPv4，IPv6中使用邻居发现协议，它被合并到ICMPv6中。

> RARP：ARP的反向映射

- ARP通过链路层广播（ARP协议的工作方式为直接交付，不需要经过任何路由器）发起ARP请求报文，询问IP对应的MAC地址
    - 链路层广播地址(ff:ff:ff:ff:ff:ff)
    - ARP请求报文需要携带：源IP，源MAC，目的IP，目的MAC(0)
- 广播域中的所有设备搜到请求报文后检查自己的IP是否是对应的IP，如果是的话，将自己的MAC地址直接发送会询问方
    - ARP应答需要填充自己的MAC地址，将发送方和接收方的地址互换，将协议中的Op（标识是请求还是应答报文）字段设置为2，然后发送生成的应答。
- 询问方使用MAC地址发送数据报

#### ARP缓存和arp命令
ARP通过维护一份ARP缓存（每个网卡接口的网络层地址-> MAC地址的表）来实现高效的运行，ARP缓存具有超时机制，通常完整的表项超时时间为20分钟，可以使用`arp`命令查看：

```bash
# 查看所有arp缓存表项
$ arp -a
# 结果分别为
# 主机名 IP地址 MAC地址 网络接口(网卡) 硬件地址类型[ethernet]
bogon (192.168.1.1) at 5c:9:79:79:e5:cf on en0 ifscope [ethernet]
bogon (192.168.1.4) at 40:a3:cc:f6:1f:f8 on en0 ifscope [ethernet]
bogon (192.168.1.160) at ce:89:ce:10:e7:ab on en0 ifscope [ethernet]
? (224.0.0.251) at 1:0:5e:0:0:fb on en0 ifscope permanent [ethernet]
? (239.255.255.250) at 1:0:5e:7f:ff:fa on en0 ifscope permanent [ethernet]
```

#### ARP 协议报文
[networksorcery](http://www.networksorcery.com/enp/protocol/arp.htm#Hardware%20type)

#### 代理 ARP
代理ARP使一个系统（通常为路由器）可以应答不同主机的ARP请求，它使ARP请求的发送者认为作出响应的就是目的主机，但实际上目的主机可能在其他地方。
...
- **IGP(interior gateway protocol)** 自治系统内部的路由选择协议（内部网关协议
